...
void TestSceneTrace(in float3 rayPos, in float3 rayDir, inout SRayHitInfo hitInfo)
{
    ...
    // light
    {
        ...
        {
            hitInfo.material = GetZeroedMaterial();
            hitInfo.material.emissive = /*$(Variable:MainLightColor)*/ * /*$(Variable:MainLightPower)*/;
    ...
    if (/*$(Variable:Scene)*/ == 0)
    {
		...
		for (int sphereIndex = 0; sphereIndex < c_numSpheres; ++sphereIndex)
		{
			if (TestSphereTrace(rayPos, rayDir, hitInfo, float4(-18.0f + 6.0f * float(sphereIndex), -8.0f, 00.0f, 2.8f)))
			{
				...
				hitInfo.material = GetZeroedMaterial();
				hitInfo.material.albedo = float3(0.9f, 0.7f, 0.25f);
				hitInfo.material.emissive = float3(0.0f, 0.0f, 0.0f);
				hitInfo.material.specularChance = 0.02f;
				hitInfo.material.specularRoughness = r/2.0f;
    ...
    else if (/*$(Variable:Scene)*/ == 6)
    {
		...
		for (int sphereIndex = 0; sphereIndex < c_numSpheres; ++sphereIndex)
		{
			if (TestSphereTrace(rayPos, rayDir, hitInfo, float4(-18.0f + 6.0f * float(sphereIndex), -8.0f, 00.0f, 2.8f)))
			{
				...
				hitInfo.material = GetZeroedMaterial();
				hitInfo.material.albedo = /*$(Variable:MaterialAlbedo)*/;
				//hitInfo.material.emissive = /*$(Variable:MaterialEmissive)*/;
				hitInfo.material.specularChance = /*$(Variable:SpecularChance)*/;
				hitInfo.material.specularRoughness = /*$(Variable:SpecularRoughness)*/;
				hitInfo.material.specularColor = /*$(Variable:SpecularColor)*/* 0.8f;
				hitInfo.material.IOR = 1.0f;
				hitInfo.material.refractionChance = /*$(Variable:RefractionChance)*/;
				hitInfo.material.refractionRoughness = 0.7f;
				hitInfo.material.refractionColor = float3(0.3f, 0.0f, 0.0f);
			}
        }
    }
}
